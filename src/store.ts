...
const idleStatus = (characterId: CharacterId): Record<string, PartStatus> =>
  Object.fromEntries(
    getCharacter(characterId).catalog.map((c) => [c.id, 'idle']),
  );
...
export const useAvatarStore = create<AvatarState>((set) => ({
  ...
  setMeshVisible: (name, visible) =>
    set((s) => ({
      meshInfos: s.meshInfos.map((m) =>
        m.name === name ? { ...m, visible } : m,
      ),
    })),
  ...
  setMeshLitColor: (name, color) =>
    set((s) => ({
      meshInfos: s.meshInfos.map((m) =>
        m.name === name ? { ...m, litColor: color } : m,
      ),
    })),
  ...
  setMeshShadeColor: (name, color) =>
    set((s) => ({
      meshInfos: s.meshInfos.map((m) =>
        m.name === name ? { ...m, shadeColor: color } : m,
      ),
    })),
}));
