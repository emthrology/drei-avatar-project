...
function graftAuxBones(
  partScene: THREE.Object3D,
  baseVrm: VRM,
): THREE.Object3D[] {
  ...
  partScene.traverse((o) => {
    ...
    if (
      b.isBone &&
      !baseBones.has(b.name) &&
      b.parent &&
      baseBones.has(b.parent.name)
    )
      roots.push(b);
  });
...
function rebindToBase(
  ...
  const skeleton = new THREE.Skeleton(
    bones,
    orig.boneInverses.map((m) => m.clone()),
  );
...
function convertPartToMToon(mesh: THREE.Mesh): void {
  ...
  const converted = list.map((m) =>
    m instanceof MToonMaterial ? m : toMToon(m),
  );
  list.forEach((m) => {
    if (!(m instanceof MToonMaterial)) m.dispose();
  });
...
export async function loadSpringPart(
  url: string,
  baseVrm: VRM,
): Promise<LoadedSpringPart> {
  const loader = new GLTFLoader();
...
export async function loadFacePart(
  url: string,
  baseVrm: VRM,
): Promise<LoadedFacePart> {
  ...
  const partVrm: VRM = (gltf as any).userData?.vrm;
  if (!partVrm)
    throw new Error('얼굴 파츠가 VRM 이 아님 (MToon 보존 위해 VRM 필요)');
  ...
  for (const sm of newFaceMeshes) {
    for (const b of sm.skeleton.bones) {
      if (!baseBoneNames.has(b.name) && !grafting.has(b.name))
        grafting.set(b.name, b);
    }
  }
  if (baseHead) {
  ...
  const donor =
    baseFaceMeshes.find((bm) => bm.morphTargetInfluences?.length) ?? null;
  const pairs: { base: THREE.SkinnedMesh; neo: THREE.SkinnedMesh }[] = [];
  const unpaired: string[] = [];
  for (const neo of meshes) {
    if (!neo.morphTargetInfluences) continue;
    const exact = baseFaceMeshes.find(
      (bm) => matNameOf(bm) === matNameOf(neo) && bm.morphTargetInfluences,
    );
  ...
  if (unpaired.length)
    console.warn(
      `[face] base 짝 없는 메시 ${unpaired.length} → donor 폴백 미러: ${unpaired.join(', ')}`,
    );
  ...
  const baseRightEye = baseVrm.humanoid.getRawBoneNode(
    VRMHumanBoneName.RightEye,
  );
  const graftLeftEye =
    graftedBones.find((b) => /_L_FaceEye/.test(b.name)) ?? null;
  const graftRightEye =
    graftedBones.find((b) => /_R_FaceEye/.test(b.name)) ?? null;
  const eyePairs = (
    [
      [baseLeftEye, graftLeftEye],
      [baseRightEye, graftRightEye],
    ] as const
  ).filter(([b, g]) => b && g) as [THREE.Object3D, THREE.Object3D][];
  ...
  const dispose = () => {
    ...
    baseFaceMeshes.forEach((m) => {
      m.visible = true;
      delete m.userData[SHADOWED_BY_PART];
    }); // base 얼굴 복원 + 표식 해제
  };
