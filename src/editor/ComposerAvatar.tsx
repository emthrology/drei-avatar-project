...
export function ComposerAvatar({ baseUrl, catalog }: Props) {
  ...
  const recollect = useCallback(
    (base: VRM) => setMeshInfos(collectMeshInfos(base)),
    [setMeshInfos],
  );

  const { vrm, vrmRef, syncFace } = useAssembledVrm(
    baseUrl,
    catalog,
    recollect,
  );
...
function frameUpperBody(
  ...
  const dist =
    (verticalSpan / 2 / Math.tan((camera.fov * Math.PI) / 180 / 2)) * 1.1;
...
function applyRestPose(vrm: VRM) {
  const armL = vrm.humanoid.getNormalizedBoneNode(
    VRMHumanBoneName.LeftUpperArm,
  );
  const armR = vrm.humanoid.getNormalizedBoneNode(
...
function collectMeshInfos(vrm: VRM): MeshInfo[] {
  ...
  vrm.scene.traverse((obj) => {
    ...
    infos.push({
      ...
      shadeColor: m?.shadeColorFactor
        ? threeColorToHex(m.shadeColorFactor)
        : '#888888',
    });
  });
