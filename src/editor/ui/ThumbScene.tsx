...
const PADDING: Record<PartCategory, number> = {
  face: 0.9,
  hair: 0.95,
  tops: 1.12,
  bottoms: 1.12,
...
const CROP_HALF_X: Partial<Record<PartCategory, number>> = {
  tops: 0.22,
  bottoms: 0.22,
};
...
function FitCamera({
  ...
  useEffect(() => {
    ...
    object.traverse((o) => {
      ...
      const pos =
        m.isMesh && m.geometry ? m.geometry.attributes.position : null;
      if (!pos) return;
      ...
      for (let i = 0; i < n; i++) {
        v.fromBufferAttribute(pos, idx ? idx.getX(i) : i).applyMatrix4(
          m.matrixWorld,
        );
        box.expandByPoint(v);
      }
    ...
    const dist =
      (sphere.radius / Math.sin((cam.fov * Math.PI) / 180 / 2)) *
      PADDING[category];
