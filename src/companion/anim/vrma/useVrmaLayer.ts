...
function loadVrma(url: string): Promise<VRMAnimation> {
  ...
  if (!p) {
    ...
    loader.register((parser: any) => new VRMAnimationLoaderPlugin(parser as any))
...
export function useVrmaLayer(vrmRef: React.RefObject<VRM | null>, opts: Options = {}) {
  ...
  useEffect(() => {
    if (!enabled) return
    const onWave = () => { pendingRef.current = VRMA_WAVE }
    // 임의 클립 재생 — 실험 패널(애셋 물색)에서 def 를 통째로 넘긴다
    const onVrma = (e: Event) => {
      const d = (e as CustomEvent).detail
      const def: VrmaClipDef | undefined =
        typeof d?.id === 'string' && !d.url ? VRMA_CLIPS[d.id] : (d as VrmaClipDef)
      if (def?.url) pendingRef.current = def
    }
    const onStop = () => { playingRef.current = null; mixerRef.current?.stopAllAction() }
  ...
  const start = async (def: VrmaClipDef) => {
    ...
    try {
      ...
      if (mixerRef.current && mixerVrmRef.current !== vrm) mixerRef.current = null
      if (!mixerRef.current) {
  ...
  useFrame((_, delta) => {
    ...
    const w = Math.min(smootherstep(p.t / p.fadeIn), smootherstep((p.dur - p.t) / p.fadeOut))
    if (w < 1) {
      const back = 1 - w
      for (let i = 0; i < p.bones.length; i++) p.bones[i].quaternion.slerp(p.snap[i], back)
    }
    for (let i = 0; i < p.bones.length; i++) p.prevOut[i].copy(p.bones[i].quaternion)
    ...
    if (p.t >= p.dur) {
      ...
      p.action.stop()
      for (let i = 0; i < p.bones.length; i++) p.bones[i].quaternion.copy(p.prevOut[i])
