...
export class AnimScheduler {
  ...
  constructor(private baseline: Record<string, number>, motion: MotionConfig = {}) {
    this.live = { ...baseline }
  ...
  private factory(template: AnimTemplate, loop: boolean): Clip {
    ...
    const baseEasing = a.ease !== undefined ? sigmoidFactory(a.ease) : this.easing
    ...
    return { name: a.name, ts: absTs, vs, offsets, chEase, maxOffset, loop, template }
  }
    ...
    for (let i = 0; i < this.queue.length; i++) {
      ...
      for (const ch of Object.keys(clip.vs)) {
        ...
        if (j >= last) {
          ...
          const alpha = span > 0.0001 ? clip.chEase[ch]((et - clip.ts[j]) / span) : 1
