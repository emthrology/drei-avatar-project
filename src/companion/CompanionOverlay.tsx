...
function DprGovernor() {
  ...
  return (
    <PerformanceMonitor onIncline={handleChange} onDecline={handleChange} />
  );
}
...
export function CompanionOverlay({
  ...
  const [status, setStatus] = useState<'loading' | 'ready' | 'speaking'>(
    'loading',
  );
  ...
  const handleReaction = useCallback(
    async (reaction: Reaction, nextMood: MoodName) => {
