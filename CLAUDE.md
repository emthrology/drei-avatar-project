...
- [ ] **▶ 진행 중: 절차 레이어 리뉴얼 (VRMA 대비 부드러움 격차)** — 브랜치 `feat/motion-renewal`. 계획·실측 [docs/motion-renewal-plan.md](docs/motion-renewal-plan.md)
  ...
  - **단계**: 0 측정 하네스(`npm run motion:stat`) ✅ → 1 보간 수정 ✅ → 2 `armRelaxed` 분포 ✅ → 5 타이밍 폴리싱 1차 ✅ → 3 본 커버리지 ✅(2026-08-10, 구동 본 7→11) → **▶ 다음: 4 연속 신호화 — 착수 전 게이트**(설계 변경이라 "0~3단계 뒤에도 아직 로봇 같은가"부터 육안 확인. 아니면 안 한다=원칙②). 각 1 PR
