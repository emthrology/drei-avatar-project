...
- [x] **색상 세트 (헤어/눈동자)** (2026-08-12) — 부위 여러 개에 **배색 한 쌍씩**을 한 번에 거는 틴트 축([colorSets.ts](src/editor/colorSets.ts)). 한 부위만 칠하면 안 되는 게 요구였다(홍채만 칠하면 하이라이트가 흰색으로 남아 겉돎). 부산물로 **눈색 이중 축 제거** — `store.eyeColor`+`partLoader.setEyeColor` 를 흡수해 색 적용을 `applyAppearance` 하나로 통일(위 불변식). ⚠️ **lit/shade 는 절대색이 아니라 텍스처 곱수라 어둡게만 가고 밝게는 못 간다** — 밝은 쪽은 틴트가 아니라 **텍스처 스왑**이라는 별도 축이다. 나머지 함정(부위 실재가 파츠마다 다름·실측값)은 `colorSets.ts` 주석
