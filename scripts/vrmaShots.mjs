...
function waitForServer(proc) {
  return new Promise((resolve, reject) => {
    const onData = (d) => { if (/Local:.*http/.test(d.toString())) resolve() }
...
async function capture(page, canvas, label, trigger, triggerArg) {
  const logs = []
  const onLog = (m) => { const t = m.text(); if (t.includes('[vrma]')) logs.push(t) }
  ...
  for (let i = 0; i < FRAMES; i++) {
    ...
    await canvas.screenshot({ path: `${OUT}/${label}_f${String(i).padStart(2, '0')}_${String(ms).padStart(4, '0')}ms.png` })
  }
...
async function main() {
  ...
  const server = spawn('npx', ['vite', '--port', String(PORT), '--strictPort'], { stdio: 'pipe', cwd: REPO })
  let browser
  try {
    ...
    await page.goto(`http://localhost:${PORT}/?mode=companion&vrma=1${CHAR ? `&char=${CHAR}` : ''}`, { waitUntil: 'networkidle0' })
    await page.waitForFunction("document.body.innerText.includes('ready')", { timeout: 60000 }).catch(() => {})
    await new Promise((r) => setTimeout(r, 2500))
    await page.evaluate(() => {
      for (const el of document.querySelectorAll('div')) {
        const st = getComputedStyle(el)
        if (st.position === 'fixed' && Number(st.zIndex) >= 1000 && !el.querySelector('canvas')) el.style.display = 'none'
      }
    })
    ...
    if (FILE) {
      ...
      for (const url of FILE.split(':')) {
        ...
        await capture(
          page, canvas, url.split('/').pop().replace('.vrma', ''),
          (o) => window.dispatchEvent(new CustomEvent('companion:vrma', {
            detail: { id: 'f', label: 'f', drop: ['hipsPosition'], fadeIn: 200, fadeOut: 300, ...o },
          })),
          { url, from: from ? Number(from) : undefined, to: to ? Number(to) : undefined },
        )
      ...
      for (let i = 0; i < n; i++) {
        const keep = (keeps[Math.min(i, keeps.length - 1)] ?? '').split(',').filter(Boolean)
        const relative = (rels[Math.min(i, rels.length - 1)] ?? '').split(',').filter(Boolean)
        const label = `k-${keep.join('+') || 'none'}_r-${relative.join('+') || 'none'}`
        console.log(`\n▶ keep=[${keep.join(' ')}] relative=[${relative.join(' ')}]`)
        await capture(
          page, canvas, label,
          (o) => window.dispatchEvent(new CustomEvent('companion:vrma', {
            detail: { id: 'probe', label: 'probe', url: '/animations/VRMA_02.vrma', from: 2.6, to: 5.6, fadeIn: 260, fadeOut: 420, ...o },
          })),
          { keep, relative },
        )
        await new Promise((r) => setTimeout(r, 2500))
      ...
      console.log('\n▶ 절차 손인사 (기준선)')
      await capture(page, canvas, 'proc', () => window.dispatchEvent(new Event('companion:wave-proc')))
      await new Promise((r) => setTimeout(r, 2500))

      console.log('\n▶ VRMA 손인사 (정식) — 뒤쪽 컷은 idle 복귀 확인용')
      await capture(page, canvas, 'vrma', () => window.dispatchEvent(new Event('companion:wave')))
    }
    console.log(`\n📁 ${OUT}`)
  } finally {
