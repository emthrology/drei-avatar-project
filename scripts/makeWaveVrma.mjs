...
function sampler(g, bone, path = 'rotation') {
  const node =
    g.json.extensions.VRMC_vrm_animation.humanoid.humanBones[bone]?.node;
...
console.log(
  `편 손 자세 추출: ${Object.keys(openHand).length}본 (${HAND_SRC.split('/').pop()} @ ${HAND_SRC_T}s)`,
);
...
{
  const handNode =
    g.json.extensions.VRMC_vrm_animation.humanoid.humanBones.rightHand.node;
  ...
  for (let i = 0; i < t.length; i++) {
    const rad =
      ((AMP * Math.PI) / 180) * Math.sin(2 * Math.PI * FREQ * (t[i] - t0));
  ...
  console.log(
    `손목 흔들기 주입: 월드축=${AXIS} amp=${AMP}° freq=${FREQ}Hz (${t.length}프레임)`,
  );
}
