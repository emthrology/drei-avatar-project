...
const STAMP_FILE = join(CACHE, 'dev-server.json')
...
const IDLE_KILL_MS = Math.max(60_000, Number(process.env.PROBE_IDLE_MS ?? 15 * 60 * 1000))
...
/**
 * dev 서버를 **신원으로** 찾는다. 포트 번호만 믿으면 5173 을 선점한 다른 프로젝트의 서버를
 * 조용히 측정하게 된다 (크롬 9222 에서 실제로 당한 실수와 같은 종류).
 * vite.config.ts 의 probe-dev-server-stamp 플러그인이 남긴 root/pid 를 확인한다.
 */
function findDevServer() {
  if (PORT_OVERRIDE) return { port: Number(PORT_OVERRIDE), why: '--port 지정' }
  let stamp
  try {
    stamp = JSON.parse(readFileSync(STAMP_FILE, 'utf8'))
  } catch {
    throw new Error('dev 서버를 찾을 수 없다 — `npm run dev` 를 먼저 실행할 것')
  }
  if (resolve(stamp.root) !== REPO) {
    throw new Error(`다른 저장소의 dev 서버다 (${stamp.root}) — 붙지 않는다`)
  }
  if (!alive(stamp.pid)) {
    throw new Error('dev 서버가 죽어 있다 (기록만 남음) — `npm run dev` 를 다시 실행할 것')
  }
  return { port: stamp.port, why: `신원 확인됨 pid ${stamp.pid}` }
}
...
async function acquireBrowser() {
  ...
  try {
    ...
    if (!cmd.includes(PROFILE_DIR)) {
      throw new Error(`포트 ${DEBUG_PORT} 를 남의 브라우저가 쓰고 있다 (PID ${owner}) — 붙지 않는다`)
    }
...
function report(r) {
  ...
  for (const c of r.checks) {
    console.log(`  ${c.pass ? '✓' : '✗'} ${c.name.padEnd(12)} ${String(c.value).padStart(10)}   기대 ${c.want}`)
  }
  console.log('─'.repeat(58))
  if (r.tipSpan) {
    console.log(
...
async function main() {
  ...
  const { port, why: portWhy } = findDevServer()
  console.log(`  dev 서버 :${port} (${portWhy})`)

  const { browser, launched } = await acquireBrowser()
  console.log(`  ${launched ? '브라우저 기동' : '기존 브라우저 재사용'} (${Date.now() - t0}ms)`)
  ...
  if (page.url() !== url) why = 'URL 다름'
    ...
    const loadedAt = await page.evaluate(() => performance.timeOrigin).catch(() => 0)
    ...
    if (srcMtime > loadedAt) why = `소스가 페이지보다 ${((srcMtime - loadedAt) / 1000).toFixed(0)}초 새로움`
  }
  if (why) {
    console.log(`  리로드 (${why})`)
  ...
  // 실측 원인: 로드 중간에 브라우저가 죽으면 프로필 캐시가 오염돼 계속 loading 에 머문다.
  await page
    .waitForFunction(
    ...
    for (const i of String(GESTURE).split(',')) {
      results.push(
        await runOnce(
          () =>
            page.evaluate((n) => {
              window.dispatchEvent(new CustomEvent('companion:gesture', { detail: { index: Number(n) } }))
            }, i),
          `제스처 ${i}`,
        ),
      )
  ...
  process.exitCode = results.length && results.every((r) => r && r.pass) ? 0 : 1
}
